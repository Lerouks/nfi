...
import { useMarketData, useChartData } from "../../lib/siteData";
    ...
    <div className="bg-white rounded-xl border p-5 shadow-sm" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-gray-400">Évolution sur {chartData.brvm.length} mois</p>
        </div>
        <div className="text-right">
          ...
          <div className={`text-xs font-semibold ${brvm ? (brvm.change_abs >= 0 ? "text-green-600" : "text-red-600") : "text-green-600"}`}>
            {brvm ? brvm.change_pct : "+0.68%"}
          </div>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={160}>
        <LineChart data={chartData.brvm} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
  ...
  return (
    <div className="bg-white rounded-xl border p-5 shadow-sm" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={chartData.gdpGrowth} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
  ...
  return (
    <div className="bg-white rounded-xl border p-5 shadow-sm" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <BarChart data={chartData.investment} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
